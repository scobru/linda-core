...
export class CommunicationService {
    ...
    pubkeyCache = new Map();
    epubCache = new Map();
    secretCache = new Map(); // Memoized DH secrets
    inboxCertCache = new Map(); // Memoized Zen certs
    myPair = null;
    cryptoMutex = Promise.resolve(); // Serialize all WebCrypto operations
    pubkeyPromises = new Map();
    epubPromises = new Map();
    inboxCertPromises = new Map();
    constructor(db) {
        this.db = db;
    }
    get initialized() {
     ...
     */
    async initSession(username, uniqueUsername) {
        if (this.isInitialized)
            return;
        if (this.initPromise)
            return this.initPromise;
        this.initPromise = (async () => {
            ...
            try {
                ...
                this.persistAlias(username, uniqueUsername).catch((e) => {
                    console.warn("[CommunicationService] Background alias persistence failed:", e);
                });
            }
            catch (e) {
     ...
     */
    async publishBundle(username, uniqueUsername) {
        const pair = this.myPair || this.db.pair;
        if (!pair || !pair.pub) {
            console.warn("[CommunicationService] User keys not available yet, deferring bundle publish.");
            return;
        }
        try {
            ...
            console.log("[CommunicationService] Publishing secondary linda_bundle_v7 metadata...");
            ...
            console.log("[CommunicationService] Published Zen bundle properties successfully.");
        }
        catch (e) {
            console.error("[CommunicationService] GunDB error during bundle publish:", e.message || e);
        }
    }
    /**
     * Persists the user's alias and unique username for discovery.
     */
    async persistAlias(username, uniqueUsername) {
        ...
        try {
            const aliasPayload = { alias: username };
            if (uniqueUsername)
                aliasPayload.uniqueUsername = uniqueUsername;
            ...
            await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error("Alias put timeout")), aliasTimeout)),
            ]);
            ...
            if (uniqueUsername) {
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Unique username put timeout")), aliasTimeout)),
                ]);
            }
        }
        catch (e) {
            console.warn("[CommunicationService] Failed to persist alias to GunDB (possibly slow relay or timeout):", e);
        }
    }
    /**
     * Resolves a human-readable username or unique username to a GunDB public key.
     */
    async getPubKeyFromUsername(username) {
        if (!username)
            throw new Error("Username/Pubkey is required");
        ...
        const promise = (async () => {
            ...
            for (let i = 0; i < 6; i++) {
                try {
                    ...
                    const data = (await this.db.Get(`~@${loginQuery}`));
     ...
     */
    async getEpubFromPub(pub) {
        if (!pub)
            throw new Error("Pubkey is required for epub fetch");
        ...
        if (existing)
            return existing;
        const promise = (async () => {
            console.log(`[CommunicationService] Discovery: Fetching epub for: ${pub.slice(0, 8)}...`);
            ...
            for (let i = 0; i < 5; i++) {
                try {
                    ...
                    const gunEpub = await new Promise((resolve) => {
                    ...
                    if (gunEpub) {
                        console.log(`[CommunicationService] Found epub via direct Gun node for: ${pub.slice(0, 8)}`);
                    ...
                    if (bundle &&
                        ...
                        console.log(`[CommunicationService] Found epub via bundle for: ${pub.slice(0, 8)}`);
                    ...
                    const profileEpub = await new Promise((resolve) => {
                    ...
                    if (profileEpub) {
                        console.log(`[CommunicationService] Found epub via profile node for: ${pub.slice(0, 8)}`);
                    ...
                    console.warn(`[CommunicationService] Epub fetch attempt ${i + 1} for ${pub.slice(0, 8)} failed:`, e?.message || e || "Unknown error");
                }
                // Jittered exponential-ish backoff
                const backoff = Math.min(5000, 1000 * (i + 1) + Math.random() * 500);
                await new Promise((r) => setTimeout(r, backoff));
            }
            throw new Error(`Could not find Zen epub for ${pub.slice(0, 8)} after 5 attempts.`);
        })();
     ...
     */
    async regenerateCertificate(force = false) {
        const pair = this.myPair || this.db.pair;
        if (!pair) {
        ...
        try {
            if (!force) {
                let currentCert = await new Promise((resolve) => {
                    let timeout = setTimeout(() => resolve(null), 3000);
                    user.get("inbox_cert_v13").once((data) => {
                        clearTimeout(timeout);
                        resolve(data);
                    });
                });
                ...
                if (currentCert && typeof currentCert === "string") {
                    try {
                        const verified = await zenCrypto.verify(currentCert, pair.pub, this.db.zen);
                        if (verified && verified.c) {
                            // Check if the policy mentions linda_inbox_v13
                            const policyStr = JSON.stringify(verified.c);
                            if (policyStr.includes("linda_inbox_v13") ||
                                policyStr.includes('"*"')) {
                                isValid = true;
                            }
                        }
                        ...
                        console.warn("[CommunicationService] Existing certificate verification failed, will regenerate.");
                    }
                }
                if (isValid) {
                    console.log("[CommunicationService] Valid Zen inbox certificate (v13) found for current session.");
                    return;
                }
            }
            ...
            */
            ...
            console.error("[CommunicationService] Error during inbox certificate generation:", e);
        }
    }
    /**
     * Issues a specific Zen certificate for a peer.
     */
    async issueCertificate(peerPub) {
        if (!this.myPair)
            throw new Error("Not logged in");
        console.log(`[CommunicationService] Issuing specific recursive certificate for: ${peerPub.slice(0, 8)}...`);
        const soul = `~${this.myPair.pub}/linda_inbox_v13`;
        const cert = await zenCrypto.certify([peerPub], [
            { "#": { "*": soul } },
            { "#": soul },
            { "#": soul + "/" },
            { "#": { "*": soul + "/" } },
            { "#": { "*": "*" } }, // absolute wildcard
        ], this.myPair, this.db.zen);
     ...
     */
    async revokeCertificate(peerPub) {
        if (!this.db.isLoggedIn())
            return;
        console.log(`[CommunicationService] Revoking certificate for: ${peerPub.slice(0, 8)}`);
        this.db.userPut(`certs/${peerPub}`, null);
    }
    /**
     * Retrieves the Zen certificate allowing writes to a peer's linda_inbox
     */
    async getInboxCertificate(pub) {
        if (!pub)
        ...
        const promise = (async () => {
            ...
            console.log(`[CommunicationService] Discovery: Fetching inbox certificate for: ${pub.slice(0, 8)}...`);
            // Helper: validate that a cert's policy actually covers linda_inbox_v13
            const validateCert = async (cert, label) => {
                try {
                    const verified = await zenCrypto.verify(cert, pub, this.db.zen);
                    if (!verified || !verified.c) {
                        console.warn(`[CommunicationService] ${label} cert for ${pub.slice(0, 8)} failed Zen verify`);
                        return false;
                    }
                    const policyStr = JSON.stringify(verified.c);
                    ...
                    console.warn(`[CommunicationService] ${label} cert for ${pub.slice(0, 8)} has incompatible policy: ${policyStr.substring(0, 80)}`);
                    return false;
                }
                catch (e) {
                    console.warn(`[CommunicationService] ${label} cert validation error for ${pub.slice(0, 8)}:`, e);
                    return false;
                }
            };
            for (let i = 0; i < 10; i++) {
                try {
                    ...
                    if (myPub) {
                        const specificCert = await new Promise((resolve) => {
                        ...
                        if (specificCert && (await validateCert(specificCert, "specific"))) {
                            console.log(`[CommunicationService] Found valid specific certificate for ${pub.slice(0, 8)}`);
                            this.inboxCertCache.set(pub, specificCert);
                            return specificCert;
                        }
                    }
                    // Method 2: Public certificate v13 (latest) — validated
                    const v13Cert = await new Promise((resolve) => {
                        const timeout = setTimeout(() => resolve(null), 3000);
                        this.db.zen
                            .get(`~${pub}`)
                            .get("inbox_cert_v13")
                    ...
                    if (v13Cert && (await validateCert(v13Cert, "v13"))) {
                        console.log(`[CommunicationService] Found valid v13 certificate for ${pub.slice(0, 8)}`);
                        this.inboxCertCache.set(pub, v13Cert);
                        return v13Cert;
                    }
                    // Method 3: Public certificate in bundle v8 — validated
                    const bundleCert = await new Promise((resolve) => {
                        const timeout = setTimeout(() => resolve(null), 3000);
                        this.db.zen
                            .get(`~${pub}`)
                            .get("linda_bundle_v8")
                    ...
                    if (bundleCert && (await validateCert(bundleCert, "bundle_v8"))) {
                        console.log(`[CommunicationService] Found valid bundle v8 certificate for ${pub.slice(0, 8)}`);
            ...
            throw new Error(`Could not find valid Zen inbox certificate for ${pub.slice(0, 8)} after multiple attempts.`);
        })();
     ...
     */
    async encryptMessage(recipientUsernameOrPub, message) {
        return new Promise((resolve, reject) => {
            this.cryptoMutex = this.cryptoMutex.then(async () => {
                try {
                    ...
                    const myPair = this.db.user?._?.sea;
                    if (!myPair)
                    ...
                    if (!secret) {
                        ...
                        if (secret)
                            this.secretCache.set(pubKey, secret);
                    }
                    if (!secret)
                        throw new Error("DH Derivation failed");
                    if (message.startsWith("{")) {
                        console.log(`[CommunicationService] Encrypting metadata payload (length: ${message.length})`);
                    }
     ...
     */
    async decryptMessage(senderUsernameOrPub, ciphertext, _senderEpub) {
        try {
            let pubKey = senderUsernameOrPub;
            if (pubKey.length < 30 || pubKey.startsWith("@")) {
                pubKey = await this.getPubKeyFromUsername(senderUsernameOrPub);
            }
            const myPair = this.myPair || this.db.user?._?.sea;
            if (!myPair)
                throw new Error("User keys not available for decryption");
            if (typeof ciphertext.body !== "string") {
                console.warn(`[CommunicationService] Body of message from ${pubKey.slice(0, 8)} is not a string (${typeof ciphertext.body}). Skipping decryption.`);
                return undefined;
            }
            ...
            if (!secret) {
                console.warn(`[CommunicationService] Could not derive secret for ${pubKey.slice(0, 8)}`);
                return undefined;
            }
            console.log(`[CommunicationService] Derived secret for ${pubKey.slice(0, 8)}. Calling Zen decrypt... (cipher length: ${ciphertext.body.length})`);
            let decrypted;
            try {
                decrypted = await Promise.race([
                    zenCrypto.decrypt(ciphertext.body, secret, this.db.zen),
                    new Promise((_, reject) => setTimeout(() => reject(new Error("Zen decrypt timeout")), 5000)),
                ]);
                console.log(`[CommunicationService] Zen decrypt resolved. decrypted value type: ${typeof decrypted}`);
            }
            catch (decryptErr) {
                console.error(`[CommunicationService] Zen decrypt threw or timed out:`, decryptErr.message);
                decrypted = undefined;
            }
            if (decrypted === undefined || decrypted === null) {
                console.warn(`[CommunicationService] Zen Decryption yielded ${decrypted === null ? "NULL" : "UNDEFINED"} for sender: ${pubKey.slice(0, 8)} after retry.`);
                return undefined;
            }
            ...
            if (typeof decrypted !== "string") {
                const stringified = typeof decrypted === "object"
                    ? JSON.stringify(decrypted)
                    : String(decrypted);
                console.log(`[CommunicationService] Decrypted non-string payload (${typeof decrypted}). Serialized to:`, stringified.substring(0, 50));
                return stringified;
            }
            return decrypted;
        }
        catch (err) {
            console.error(`[CommunicationService] Error during decryption for ${senderUsernameOrPub}:`, err.message);
            return undefined;
        }
    }
    /**
     ...
     */
    async republishBundle() {
        const username = localStorage.getItem("linda_alias") || "Anonymous";
        const uniqueUsername = localStorage.getItem("linda_unique_username") || undefined;
     ...
     */
    async resetSession(contactUsernameOrPub) {
        console.log(`[CommunicationService] Reset requested for ${contactUsernameOrPub} (Zen mode is stateless).`);
        // Clear cache to force fresh epub fetch
        const oldEpub = this.epubCache.get(contactUsernameOrPub);
        if (oldEpub)
            this.secretCache.delete(oldEpub);
