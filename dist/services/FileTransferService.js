...
export class FileTransferService {
    ...
    handleIncomingSignal(from, signal) {
        ...
        switch (signal.type) {
            ...
            case 'file_candidate':
                ...
                if (processedSet.has(candString))
                    return;
    ...
    async handleIceRestartOffer(senderPub, offer) {
        if (!this.pc)
            return;
        try {
            ...
            let sdpStr = typeof offer.sdp === 'string' ? offer.sdp : offer.sdp?.sdp;
    ...
    async handleAnswer(payload) {
        try {
            ...
            if (!sdpStr || typeof sdpStr !== 'string')
                throw new Error(`Invalid answer SDP string: ${JSON.stringify(payload)}`);
            await this.pc.setRemoteDescription(new RTCSessionDescription({ type: sdpType, sdp: sdpStr }));
    ...
    async offerFile(recipientPub, file, metaId) {
        if (this.pendingOps.has(metaId) || (this.currentStatus !== 'idle' && this.currentMetaId !== metaId))
            return;
    ...
    async triggerIceRestart(recipientPub, _file, metaId) {
        if (!this.pc)
            return;
        try {
    ...
    async acceptFile(senderPub, offer) {
        const metaId = offer?.metaId;
        if (!metaId || this.pendingOps.has(metaId))
            return;
        ...
        try {
            ...
            if (typeof offer.sdp === 'object' && offer.sdp !== null && typeof offer.sdp.sdp === 'string') {
                ...
                const allValues = Object.values(offer).filter(v => typeof v === 'string' && v.startsWith('v='));
